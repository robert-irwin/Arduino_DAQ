...
void w5500UDPsend(w5500packet * pac, char sockID, char * data, int len)
{
	...
	/* !!!!!!!!!!!!!!!ALTERNATIVE!!!!!!!!!!!!!!
	txBuffINC = txBuffBegin + (uint16_t)len;
	curroffset = WIZS_TX_WR0;
	offset = (char *) &curroffset;
	w5500WritepacketConstruct(pac, offset, sockID, updateTXbuff + 1, 1);
	SPI328P_MasterTransaction(pac->packet, pac->packetlen, pac->recvbuffer);
	curroffset = WIZS_TX_WR0 + 1;
	w5500WritepacketConstruct(pac, offset, sockID, updateTXbuff, 1);
	SPI328P_MasterTransaction(pac->packet, pac->packetlen, pac->recvbuffer);
	*/
	curroffset = WIZS_TX_WR0;
	offset = (char *) &curroffset;
